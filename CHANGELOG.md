...
**Beta.** The theming system goes **OKLCH-primary**: colors are defined in OKLCH end
to end — accepted as input, exposed as a typed Swift value, and shipped as the base
stylesheet's authored defaults — with hex kept only as an invisible fallback. Saturated
colors now render richer on wide-gamut (P3) displays.

**Stability:** Stable for pre-1.0 usage. One breaking change (a parameter-label rename
on the two `Contrast` helpers), flagged below.

### Added

- OKLCH color input across `swiflow theme` and the `SwiflowColor` library: every color
  flag (`--primary`, `--danger`, `--success`, `--warning`, `--info`) and the `Contrast`
  metrics now accept `oklch(L C H)` — with `%` lightness, a `deg` suffix, an optional
  `/ alpha`, and surrounding whitespace — anywhere hex was accepted. Hex still works
  everywhere. The generated file's `Generated by …` comment echoes the OKLCH seed you
  passed (quoted), so the printed command reproduces exactly what you typed.
- `SwiflowUI.Color` — a typed OKLCH color value for the `Theme` DSL:
  `Theme(.accent(.oklch(l: 0.62, c: 0.17, h: 255)))`. Also `.hex("#…")` and plain
  string literals. It is a pure CSS-string formatter; the existing string-taking
  `Theme` / `ThemeToken` overloads are unchanged.

### Changed

- SwiflowUI's base stylesheet authors its accent, status, and neutral colors in
  absolute `oklch()` rather than hex. On a wide-gamut (P3) display these render with
  fuller saturation; on sRGB they gamut-map to the previous appearance. The now-
  redundant `@media (color-gamut: p3)` upgrade block is removed — absolute `oklch()`
  adapts to the display's gamut natively. The registered `@property` hex initial-values
  remain as the pre-`oklch()` fallback.
- **BREAKING:** the two `SwiflowColor.Contrast` helpers dropped the `Hex` suffix from
  their parameter labels now that they accept `oklch()` or hex:
  `Contrast.apca(textHex:bgHex:)` → `Contrast.apca(text:bg:)`, and
  `Contrast.wcag(_ aHex:_ bHex:)` → `Contrast.wcag(_ a:_ b:)`. Update call sites.

### Fixed

- Generated themes are now contrast-validated in **both** the sRGB and Display-P3
  renderings, not only P3. A chroma widening that cleared WCAG on a P3 display but would
  fall below it once gamut-mapped to sRGB is now backed off (output is unchanged for
  seeds that were already safe in both gamuts).
