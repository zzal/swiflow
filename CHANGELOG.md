...
- **CSS DSL — `raw(_:)`** escape hatch. Emits its string verbatim with no scoping. Used for at-rules the DSL doesn't model (`@property`, `@container`). Deliberate small surface; dedicated builders land when a specific at-rule becomes common.
