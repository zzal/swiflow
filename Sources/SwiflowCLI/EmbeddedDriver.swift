...
(function(){"use strict";const r=new Map,u=new Map;let g=null;const y=new Map,_="./.build/plugins/PackageToJS/outputs/Package/App.wasm";function v(e){const t=document.createElement("template");if(t.innerHTML=e,t.content.childNodes.length===1)return t.content.firstChild;const n=document.createElement("span");for(;t.content.firstChild;)n.appendChild(t.content.firstChild);return n}function b(e){const t=e.target,n=t instanceof Element?t:t&&t.parentElement||null;if(!n||!n.closest||!e.currentTarget)return!1;const o=n.closest("a[href],button,input,select,textarea,summary,label,[contenteditable]:not([contenteditable='false'])");return!!(o&&o!==e.currentTarget&&e.currentTarget.contains&&e.currentTarget.contains(o))}function x(e){const t=e.target,n=t&&"value"in t?String(t.value):null,o=t&&"checked"in t?!!t.checked:null,s=typeof e.key=="string"?e.key:null;return{type:e.type,targetValue:n,targetChecked:o,isSelfTarget:t===e.currentTarget,fromInteractiveDescendant:b(e),key:s,shiftKey:!!e.shiftKey,ctrlKey:!!e.ctrlKey,altKey:!!e.altKey,metaKey:!!e.metaKey,detail:e.detail!==null&&typeof e.detail=="object"?JSON.stringify(e.detail):null}}function k(e){switch(e.op){case"createElement":r.set(e.handle,document.createElement(e.tag));return;case"createText":r.set(e.handle,document.createTextNode(e.text));return;case"createRawHTML":{r.set(e.handle,v(e.html));return}case"destroyNode":{const t=r.get(e.handle);for(const n of Array.from(u.keys())){const o=n.indexOf(":");if(o<0||Number(n.slice(0,o))!==e.handle)continue;const s=n.slice(o+1),w=u.get(n);t!==void 0&&w!==void 0&&t.removeEventListener(s,w),u.delete(n)}r.delete(e.handle);return}case"animateExit":{const t=r.get(e.handle),n=r.get(e.parentHandle);if(!t)return;t.style.animation=e.animation,setTimeout(function(){n&&t.parentNode===n?n.removeChild(t):t.parentNode&&t.parentNode.removeChild(t),r.delete(e.handle)},e.durationMs);return}case"appendChild":r.get(e.parent).appendChild(r.get(e.child));return;case"insertBefore":r.get(e.parent).insertBefore(r.get(e.child),r.get(e.beforeChild));return;case"removeChild":r.get(e.parent).removeChild(r.get(e.child));return;case"setAttribute":r.get(e.handle).setAttribute(e.name,e.value);return;case"removeAttribute":r.get(e.handle).removeAttribute(e.name);return;case"setProperty":if(e.name==="innerHTML")throw new Error("swiflow: setProperty refuses to write the innerHTML property; use VNode.rawHTML(_:) instead");r.get(e.handle)[e.name]=e.value;return;case"removeProperty":delete r.get(e.handle)[e.name];return;case"setStyle":e.name.startsWith("--")?r.get(e.handle).style.setProperty(e.name,e.value):r.get(e.handle).style[e.name]=e.value;return;case"removeStyle":r.get(e.handle).style.removeProperty(e.name);return;case"setText":{const t=r.get(e.handle);t.data!==void 0?t.data=e.text:t.textContent=e.text;return}case"setRawHTML":{const t=v(e.html),n=r.get(e.handle);n&&n.parentNode&&n.parentNode.replaceChild(t,n),r.set(e.handle,t);return}case"addHandler":{const t=e.handlerId,n=e.handle+":"+e.event,o=u.get(n);o!==void 0&&r.get(e.handle).removeEventListener(e.event,o);const s=function(w){window.__swiflowDispatch(t,x(w))};r.get(e.handle).addEventListener(e.event,s),u.set(n,s);return}case"removeHandler":{const t=e.handle+":"+e.event,n=u.get(t);n!==void 0&&(r.get(e.handle).removeEventListener(e.event,n),u.delete(t));return}case"replaceMount":{const t=document.querySelector(e.selector);if(t===null)throw new Error("swiflow-driver: replaceMount target '"+e.selector+"' not found");const n=y.get(e.selector);n!==void 0&&n.parentNode===t&&t.removeChild(n);const o=r.get(e.newHandle);y.set(e.selector,o),t.appendChild(o);return}default:console.error("swiflow-driver: unknown opcode",e.op,e);return}}async function p(e){const t=await fetch(e);if(!t.ok)throw new Error("swiflow: fetch "+e+" failed ("+t.status+")");const n=parseInt(t.headers.get("Content-Length")||"",10),o=t.body&&t.body.getReader?t.body.getReader():null;if(!o)return document.documentElement.dataset.swiflowProgress="100",t;const s=[];let w=0;const a=Number.isFinite(n)&&n>0;try{for(;;){const{done:i,value:l}=await o.read();if(i)break;if(s.push(l),w+=l.byteLength,a){const f=Math.floor(w/n*100);document.documentElement.dataset.swiflowProgress=String(Math.min(f,99))}}}catch(i){try{await o.cancel()}catch{}throw i}document.documentElement.dataset.swiflowProgress="100";const c=new Blob(s,{type:t.headers.get("Content-Type")||"application/wasm"});return new Response(c,{headers:t.headers,status:t.status})}if(window.swiflow={applyPatches:function(e){let t=!0;for(let n=0;n<e.length;n++)try{k(e[n])}catch(o){t=!1,console.error("swiflow-driver: patch failed (op "+(e[n]&&e[n].op)+", index "+n+" of "+e.length+")",e[n],o),typeof window.__swiflowDevError=="function"&&window.__swiflowDevError(o)}return t},mount:function(e,t){const n=document.querySelector(t);if(n===null)throw new Error("swiflow-driver: mount target '"+t+"' not found");g=t,y.set(t,r.get(e)),n.appendChild(r.get(e))},nodeForHandle:function(e){const t=r.get(e);return t===void 0?null:t},registerDispatcher:function(e){}},window.SWIFLOW_DEV){let s=function(){const a=(location.protocol==="https:"?"wss://":"ws://")+location.host+"/reload",c=new WebSocket(a);c.onopen=function(){e=250},c.onmessage=function(i){let l;try{l=JSON.parse(i.data)}catch{return}if(l){if(l.type==="reload"){location.reload();return}if(l.type==="hmr-swap"){w(l);return}}},c.onclose=function(){setTimeout(s,e),e=Math.min(e*2,t)},c.onerror=function(){}};if(console.log(`[swiflow dev] For Swift source locations in stack traces,
install the Chrome C/C++ DevTools Extension:
  https://goo.gle/wasm-debugging-extension
