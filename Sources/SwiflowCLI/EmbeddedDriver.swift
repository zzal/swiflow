...
`+(a&&a.stack?a.stack:String(a)));const c=document.getElementById("__swiflow-error-overlay");c&&c.remove();const i=document.createElement("div");i.id="__swiflow-error-overlay",i.style.cssText="position:fixed;inset:0;z-index:999999;background:rgba(0,0,0,0.85);color:#fff;font-family:monospace;font-size:14px;padding:24px;overflow:auto;white-space:pre-wrap;word-break:break-word;";const l=document.createElement("div");l.style.cssText="font-size:18px;font-weight:bold;margin-bottom:16px;color:#ff6b6b;",l.textContent="\u26A0 Swiflow render error \u2014 WASM execution stopped";const u=document.createElement("pre");u.style.cssText="margin:0 0 16px;",u.textContent=a&&a.stack?a.stack:String(a);const h=document.createElement("div");h.style.cssText="color:#aaa;font-size:12px;margin-bottom:4px;",h.textContent="Install the Chrome C/C++ DevTools Extension to see Swift file:line in the stack above:";const d=document.createElement("a");d.href="https://goo.gle/wasm-debugging-extension",d.target="_blank",d.style.cssText="color:#4dabf7;font-size:12px;display:block;margin-bottom:16px;",d.textContent="https://goo.gle/wasm-debugging-extension";const m=document.createElement("button");m.style.cssText="padding:8px 16px;background:#444;color:#fff;border:none;cursor:pointer;font-size:14px;border-radius:4px;",m.textContent="Dismiss (app is frozen \u2014 reload to continue)",m.onclick=function(){i.remove()},i.appendChild(l),i.appendChild(u),i.appendChild(h),i.appendChild(d),i.appendChild(m),(document.body||document.documentElement).appendChild(i)},typeof window.requestAnimationFrame=="function"){var k=window.requestAnimationFrame.bind(window);window.requestAnimationFrame=function(a){return k(function(c){try{a(c)}catch(i){window.__swiflowDevError(i)}})}}let e=250;const t=5e3;let n=!1,r=null;async function w(a){if(n){r=a;return}n=!0;const c=performance.now();try{const i=window.__swiflow&&window.__swiflow.hmrSnapshot?window.__swiflow.hmrSnapshot():null;if(window.__swiflowPendingSnapshot=i,o.clear(),f.clear(),g){const d=document.querySelector(g);d&&d.replaceChildren()}document.querySelectorAll('style[id^="swiflow-"]').forEach(function(d){d.remove()});const l=window.swiflow&&window.swiflow.__importOverride||(d=>import(d)),{init:u}=await l(a.jsURL);await u({module:p(a.wasmURL)});const h=(performance.now()-c).toFixed(1);console.log("[swiflow] hmr-swap took "+h+"ms")}catch(i){console.warn("[swiflow] HMR swap failed, falling back to full reload:",i),location.reload();return}finally{n=!1}if(r!==null){const i=r;r=null,w(i)}}s()}window.swiflow.__boot=async function({swiflowDev:t}){if("serviceWorker"in navigator){if(t){const n=await navigator.serviceWorker.getRegistrations();for(const r of n)if(((r.active||r.installing||r.waiting)?.scriptURL??"").endsWith("/swiflow-service-worker.js"))try{await r.unregister()}catch{}if(typeof caches<"u")try{const r=await caches.keys();await Promise.all(r.filter(s=>s.startsWith("swiflow-")).map(s=>caches.delete(s)))}catch{}return}try{await navigator.serviceWorker.register("swiflow-service-worker.js")}catch(n){console.warn("swiflow: service worker registration failed",n)}}},window.swiflow.__bootForTest=window.swiflow.__boot,window.swiflow.__test_fetchWithProgress=p,(async()=>{if(!window.__SWIFLOW_SKIP_BOOT&&(await window.swiflow.__boot({swiflowDev:!!window.SWIFLOW_DEV}),!window.swiflow.__inited)){window.swiflow.__inited=!0;try{const{init:e}=await import("./.build/plugins/PackageToJS/outputs/Package/index.js");await e({module:p(_)})}catch(e){console.error("swiflow: WASM init failed",e)}}})()})();
