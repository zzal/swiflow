...
`+(i&&i.stack?i.stack:String(i)));const s=document.getElementById("__swiflow-error-overlay");s&&s.remove();const a=document.createElement("div");a.id="__swiflow-error-overlay",a.style.cssText="position:fixed;inset:0;z-index:999999;background:rgba(0,0,0,0.85);color:#fff;font-family:monospace;font-size:14px;padding:24px;overflow:auto;white-space:pre-wrap;word-break:break-word;";const c=document.createElement("div");c.style.cssText="font-size:18px;font-weight:bold;margin-bottom:16px;color:#ff6b6b;",c.textContent="\u26A0 Swiflow render error \u2014 WASM execution stopped";const u=document.createElement("pre");u.style.cssText="margin:0 0 16px;",u.textContent=i&&i.stack?i.stack:String(i);const f=document.createElement("div");f.style.cssText="color:#aaa;font-size:12px;margin-bottom:4px;",f.textContent="Install the Chrome C/C++ DevTools Extension to see Swift file:line in the stack above:";const d=document.createElement("a");d.href="https://goo.gle/wasm-debugging-extension",d.target="_blank",d.style.cssText="color:#4dabf7;font-size:12px;display:block;margin-bottom:16px;",d.textContent="https://goo.gle/wasm-debugging-extension";const p=document.createElement("button");p.style.cssText="padding:8px 16px;background:#444;color:#fff;border:none;cursor:pointer;font-size:14px;border-radius:4px;",p.textContent="Dismiss (app is frozen \u2014 reload to continue)",p.onclick=function(){a.remove()},a.appendChild(c),a.appendChild(u),a.appendChild(f),a.appendChild(d),a.appendChild(p),(document.body||document.documentElement).appendChild(a)},typeof window.requestAnimationFrame=="function"){var S=window.requestAnimationFrame.bind(window);window.requestAnimationFrame=function(i){return S(function(s){try{i(s)}catch(a){window.__swiflowDevError(a)}})}}let n=250;const r=5e3;let l=!1,w=null;async function g(i){if(l){w=i;return}l=!0;const s=performance.now();try{const a=window.__swiflow&&window.__swiflow.hmrSnapshot?window.__swiflow.hmrSnapshot():null;window.__swiflowPendingSnapshot=a;try{window.__swiflow&&typeof window.__swiflow.hmrTeardown=="function"&&window.__swiflow.hmrTeardown()}catch(d){console.warn("[swiflow] hmr teardown of previous module failed:",d)}if(o.clear(),m.clear(),y){const d=document.querySelector(y);d&&d.replaceChildren()}document.querySelectorAll('style[id^="swiflow-"]').forEach(function(d){d.remove()});const c=window.swiflow&&window.swiflow.__importOverride||(d=>import(d)),{init:u}=await c(i.jsURL);await u({module:v(i.wasmURL)});const f=(performance.now()-s).toFixed(1);console.log("[swiflow] hmr-swap took "+f+"ms")}catch(a){console.warn("[swiflow] HMR swap failed, falling back to full reload:",a),location.reload();return}finally{l=!1}if(w!==null){const a=w;w=null,g(a)}}h()}window.swiflow.__boot=async function({swiflowDev:t}){if("serviceWorker"in navigator){if(t){const n=await navigator.serviceWorker.getRegistrations();for(const r of n)if(((r.active||r.installing||r.waiting)?.scriptURL??"").endsWith("/swiflow-service-worker.js"))try{await r.unregister()}catch{}if(typeof caches<"u")try{const r=await caches.keys();await Promise.all(r.filter(l=>l.startsWith("swiflow-")).map(l=>caches.delete(l)))}catch{}return}try{await navigator.serviceWorker.register("swiflow-service-worker.js")}catch(n){console.warn("swiflow: service worker registration failed",n)}}},window.swiflow.__bootForTest=window.swiflow.__boot,window.swiflow.__test_fetchWithProgress=v,(async()=>{if(!window.__SWIFLOW_SKIP_BOOT&&(await window.swiflow.__boot({swiflowDev:!!window.SWIFLOW_DEV}),!window.swiflow.__inited)){window.swiflow.__inited=!0;try{const{init:e}=await import("./.build/plugins/PackageToJS/outputs/Package/index.js");await e({module:v(_)})}catch(e){console.error("swiflow: WASM init failed",e)}}})()})();
