...
extension Color {
    ...
    static func accentThemeCSS(primaryHex: String,
        ...
        // The header echoes the ORIGINAL seed the user typed (an oklch() input stays oklch,
        // not the normalized hex), quoted so a value with spaces — or a leading '#', which a
        // shell treats as a comment — still reproduces as a valid command.
        func echo(_ raw: String) -> String { "\"\(raw.trimmingCharacters(in: .whitespacesAndNewlines))\"" }
        ...
        if let dangerHex {
            ...
            flagEcho += " --danger \(echo(dangerHex))"
        }
        if let successHex {
            ...
            flagEcho += " --success \(echo(successHex))"
        }
        if let warningHex {
            ...
            flagEcho += " --warning \(echo(warningHex))"
        }
        if let infoHex {
        ...
        if !includeNeutrals {
            let rootBody = (tokenLines("--sw-accent", light, dark, rawBar: 3.0, textBar: 4.5) + statusLines)
                .joined(separator: "\n")
            let css = """
            /* Generated by `swiflow theme --primary \(echo(primaryHex))\(flagEcho)`. Include after SwiflowUI's styles.
        ...
        let moreLines = neutralContrastMore(accentHex: light)
            ...
            .joined(separator: "\n")
        let css = """
        /* Generated by `swiflow theme --primary \(echo(primaryHex))\(flagEcho) --neutrals`. Include after SwiflowUI's styles.
