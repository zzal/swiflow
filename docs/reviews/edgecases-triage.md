...
## Decision: not embedded as a template (deviates from spec §6)

The harness is built **in-place** (`swiflow dev --path examples/EdgeCases`, see `playwright.edgecases.config.ts`), not embedded as a `swiflow init --template EdgeCases`. Rationale: it's a deliberately-ugly internal harness (a poor `init` starting point), the e2e tests the real source directly, and not embedding keeps the curated user template list (HelloWorld, MiniRouter) clean and avoids the template-freshness maintenance burden on every trap edit. `EmbeddedTemplates.swift` is intentionally unchanged. Reversible if a user-facing template is later wanted.
